    ...
    <main className="flex min-h-screen flex-col items-center justify-center p-6 relative">
      ...
      <Card className={"w-full max-w-sm border-[var(--border)] bg-[var(--bg-card)] backdrop-blur-[var(--card-blur)] shadow-xl transition-all " + (status === "error" ? "border-[var(--status-failed)]" : "hover:border-[var(--border-hover)] hover:bg-[var(--bg-card-hover)]")}>
        <CardContent className="flex flex-col gap-5 px-6 py-8">
          <Input placeholder="Organization" value={org} onChange={e => setOrg(e.target.value)} className="bg-[rgba(15,25,50,0.6)] border-[var(--border)] rounded-[10px] text-sm h-11 text-white focus:border-[var(--accent-blue)] focus:ring-[3px] focus:ring-[rgba(74,144,217,0.15)] focus:outline-none" onKeyDown={e => e.key === "Enter" && handleLogin()} />
          <Input type="password" placeholder="API Key" value={key} onChange={e => setKey(e.target.value)} className="bg-[rgba(15,25,50,0.6)] border-[var(--border)] rounded-[10px] text-sm h-11 text-white focus:border-[var(--accent-blue)] focus:ring-[3px] focus:ring-[rgba(74,144,217,0.15)] focus:outline-none" onKeyDown={e => e.key === "Enter" && handleLogin()} />
